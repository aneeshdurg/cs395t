...
# Benchmarking Loop
RESULTS_DIR="test_results/milvus"
...
for setting in always never madvise; do
  ...
  echo ""
  BENCH_OUTPUT="$RESULTS_DIR/hnsw_results_${setting}.txt"
  PERF_OUTPUT="$RESULTS_DIR/perf_results_${setting}.txt"
  echo "Running benchmark script for..."
  ...
  conda run -n "$CONDA_ENV_NAME" perf stat \
    ...
    -o "$PERF_OUTPUT" \
    -- \
    python -u milvus_sift_benchmark.py | tee "$BENCH_OUTPUT"
